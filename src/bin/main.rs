use traceroute::{Traceroute, TracerouteError, Options};
    ...
    match error {
        TracerouteError::Io(err) => match err.kind() {
            ErrorKind::PermissionDenied => 
                eprintln!("Couldn't open network: {:?}. Try again with sudo", err.kind()),
        ...
        TracerouteError::Impossible(err) |
        TracerouteError::UnmatchedPacket(err) => eprintln!("Failed: {:?}", err),
        ...
        TracerouteError::UnimplimentedProtocol(proto) => eprintln!("No support for {:?} probes yet", proto),
    };
    std::process::exit(1);
}
